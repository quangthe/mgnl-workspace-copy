...
FROM postgres:16-alpine AS tools
...
COPY --from=postgres-15 /usr/local/bin /usr/lib/postgresql/15/bin
...
COPY --from=postgres-15 /usr/local/lib /usr/lib/postgresql/15/lib
...
COPY --from=postgres-15 /usr/local/share /usr/lib/postgresql/15/share
